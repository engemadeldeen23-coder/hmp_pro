...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _init() async {
    await [Permission.location, Permission.bluetoothScan,
        Permission.bluetoothConnect, Permission.camera].request();
  ...
  Future<void> _fetchWeather() async {
    try {
      ...
      if (pos == null) return;
      // Simple weather via wttr.in (no API key needed)
      final url = Uri.parse(
  ...
  Widget _buildUpperZone() {
    return Container(
      ...
      decoration: const BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: Color(0xFF1A2540)),
        ),
      ),
      child: Column(
        children: [
          // Row 1: Menu | Time & Date | Battery
          Row(
            children: [
              Builder(
          ...
          // Row 2: Info bar (weather + connection status)
          Container(
  ...
  Widget _buildBatteryIndicator() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            _batteryCharging ? Icons.battery_charging_full : Icons.battery_full,
            color: color,
            size: 14,
          ),
          const SizedBox(width: 4),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _statusRow('Drops', loc == null ? '—' : '${loc.drops.length}'),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _smallMetric('ACC', '${_lastAcc.toStringAsFixed(2)} g',
                    Colors.purpleAccent),
              ),
              Expanded(
                child: _smallMetric('VEL', '${_lastVel.toStringAsFixed(3)} m/s',
                    Colors.greenAccent),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: Text(_ble.isConnected ? 'CONNECTED' : 'CONNECT'),
                ...
                children: [
                  ...
                  const Text('HMP PRO',
                      style: TextStyle(
                          ...
                          fontWeight: FontWeight.bold,
                          letterSpacing: 2)),
                  Text(_profile.companyName.isEmpty
                          ? 'Settings Menu'
  ...
  Future<void> _connectDevice() async {
    ...
    if (!mounted) return;
    setState(() => _eyeState =
        ok ? MagicEyeState.blue : MagicEyeState.red);
  ...
  Future<void> _saveSitesAndActive() async {
    ...
    await _storage.saveActive(
        _activeSiteId, _activeJobId, _activeLocationId);
    setState(() {});
  }
        ...
        content: pos == null
            ...
            : Column(
                ...
                children: [
                  ...
                  _gpsRow('Accuracy', '±${pos.accuracy.toStringAsFixed(1)} m'),
                  _gpsRow('Altitude', '${pos.altitude.toStringAsFixed(1)} m'),
                ],
              ),
        actions: [
          TextButton(
...
// ============================================================
//  STUB SCREENS - to be filled below in FILE 9
// ============================================================
// (UserProfilePage, ProjectSettingsPage, DropSettingsPage,
//  SiteSettingsPage, JobSettingsPage, LocationSettingsPage,
//  DropListPage, MeasurementPage)